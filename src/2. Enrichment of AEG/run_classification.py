...
class CorefProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            [l.strip().split(',') for l in
             open(os.path.join(data_dir, "train.csv"), encoding='UTF-8')], "train", data_dir)
    ...
    def _create_examples(self, lines, set_type, data_dir):
        ...
        def id2feature(event_id):
            event_item = event_map[event_id]
            attribute_name = ['type', 'span', 'concept', 'predicate', 'modifier', 'direction']

            def ifNone(attr_name):
                attr = event_item[attr_name]
                return attr[:20] if attr else 'non'
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text_a = id2feature(line[0])
            text_b = id2feature(line[1])
            label = line[2]
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
            if len(lines) < 1000000 and 'pred' not in argv:
...
def evaluate(args, model, tokenizer, labels, prefix="", during_train=False):
    ...
    eval_outputs_dirs = (args.pred_dir if args.pred_dir else args.output_dir,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        print(classification_report(y_pred=preds, y_true=out_label_ids, target_names=labels, digits=3))
...
def features_wrapper(examples):
    wrapper = {k: v for k, v in wrapper_dict.items() if k != 'tokenizer'}
    # print(type(examples), len(examples))
    features = convert_examples_to_features(examples, wrapper_dict['tokenizer'], **wrapper)
    return features
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        csize = 1000000
        chunks = len(examples) // csize + (1 if len(examples) % csize else 0)
        result = pool.map_async(features_wrapper, (examples[i * csize:(i + 1) * csize] for i in range(chunks))).get()
        pool.close()
        pool.join()
        features = sum(result, [])
